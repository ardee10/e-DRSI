...
class M_dashboard extends CI_Model
{
	...
	public function dataFindingBd($id = null)
	{
		...
		try {
			$this->db->select('*');
			$this->db->from('tbl_finding');
			$this->db->join('tbl_defect_class', 'tbl_finding.defect_name = tbl_defect_class.id_defect_class', 'left');
			$this->db->join('tbl_defect_sub_class', 'tbl_finding.defect_name2 = tbl_defect_sub_class.id_defect_sub_class', 'left');

			// Periksa apakah $id ada dan tidak kosong
			if ($id) {
				// Gunakan parameterized query untuk mencegah SQL injection
				$this->db->where('tbl_finding.gedung', $id);
			}
			...
			if ($query->num_rows() > 0) {
				...
				return array(); // Kembalikan array kosong jika data tidak ditemukan
			}
		} catch (Exception $e) {
			// Catat error ke log
			log_message('error', $e->getMessage());
			return array(); // Kembalikan array kosong jika terjadi error
		}
	}
}
